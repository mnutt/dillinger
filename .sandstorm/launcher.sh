...
# /var is the grain's only persistent writable directory.
export HOME=/var
...
# The VM's CPU exposes Intel PKU, but Sandstorm's grain sandbox does not support
# the pkey_mprotect() path V8 selects for WebAssembly code pages. The small
# server starts without Wasm, while this flag keeps its on-demand Fetch/PDF
# features compatible with the grain as well.
exec /usr/bin/node \
  --no-wasm-memory-protection-keys \
  /opt/app/.next/sandstorm-package/server.cjs
